# ------------------------------------------------------------
# Free Fire Account Info API — Credit: @HAYATO OFC
# JOIN    : @HAYATO MODS  FOR MORE SRC | API | BOT CODE | METHOD | 🛐
# Purpose : Fetch Free Fire profile details using UID (JWT + AES)
# Note    : THIS CODE MADE BY HAYATO OFFICIAL — KEEP CREDIT
...
def get_jwt_token_sync(region):
    ...
    endpoints = {
        ...
        "default": "https://raihan-access-to-jwt.vercel.app/token?uid=4349251999&password=GARENA_KI_MKC_PZEEC_BY_HAYATO_CODEX_4P5PL"
    }
...
def index():
    return jsonify({
        "message": "Free Fire Account Info API - HAYATO CODEX",
        "endpoint": "/info?uid=PLAYER_UID&region=REGION",
        "supported_regions": ["IND", "BD", "PK"],
